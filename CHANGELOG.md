...
### 排序规则语法验证增强
- **修复"!"标志位置验证逻辑**：
  - 问题：原始验证逻辑过于严格，将有效规则如`@@C^!V*`误判为无效
  - 影响：用户无法使用包含位置指定符的有效排序规则
  - 修复：更新`validateNonGroupingPosition`函数，正确识别位置指定符(`^`, `$`, `*`, `~`)作为排序单元的一部分
  - 结果：现在正确支持所有有效的"!"标志位置，同时阻止无效位置使用

- **增强规则语法一致性**：
  - 防止"!"标志在规则末尾、括号内部或直接跟在"-"后面的无效使用
  - 提供清晰的错误提示信息，帮助用户理解正确的语法
  - 确保向后兼容性，不影响现有正确格式的规则
...
### 规则引擎重大修复和改进
- **修复:V规则匹配逻辑**：
  - 问题：原始`:V`规则使用全匹配模式，要求单词中所有字符都是元音才能匹配
  - 影响：导致包含辅音的单词（如"apple"、"hello"）无法正确匹配":V"规则
  - 修复：将`:V`规则从`matchWithBacktrack`（全匹配）改为`matchesPatternPartial`（部分匹配）
  - 结果：正确实现"包含元音"的语义，匹配单词数从171增加到230

- **支持中文字符规则名**：
  - 问题：`tokenizeLogic`方法的正则表达式`/[\w\d_]/`只支持英文字符，导致中文规则名解析失败
  - 影响：组合规则如`#元音开头且辅音结尾 ::元音开头&&辅音结尾 @`出现"缺少规则名"错误
  - 修复：将正则表达式扩展为`/[\w\d_\u4e00-\u9fff]/`，添加中文字符Unicode范围支持
  - 结果：完全支持中文规则名，增强规则引擎的国际化能力
...
### 规则引擎核心修复
- **修复回溯算法关键bug**：字符消耗验证缺失
  - 问题：`matchWithBacktrack`函数在模式匹配完成时直接返回成功，未验证所有字符是否被消耗
  - 影响：导致多量词规则如`:\b(PRE)L+V"tion"\e`错误匹配单词"attention"
  - 修复：添加字符消耗完整性检查，确保`wordIndex >= word.length`时才返回成功
  - 结果：提升规则引擎匹配准确性，解决部分字符被忽略的问题
...
### 技术细节
- 修复前：模式匹配完成后直接返回`true`，忽略未处理字符
- 修复后：检查`patternIndex >= pattern.length && wordIndex >= word.length`
- 测试案例："attention"匹配`L+V`时，"tten"中的"n"不再被忽略

## [v1.2.6] - 2025-06-10

### 预处理模块重大改进
- **严格音标验证系统**：重新设计音标检查机制
  - 实现严格的英语IPA音标字符集验证，仅允许指定的IPA符号
  - 添加英语IPA元音和辅音：`iɪeɛæaɑɔoʊuʌəɜɝ` 和 `pbtdkɡfvθðszʃʒhmnŋlrjw`
  - 支持超音段符号：`ˈˌː.‿` 和结构符号：`()`
  - 新增特殊放宽字符支持：`grtʃdʒ:'a`
  - 添加音标混用检测：禁止 `ː` 与 `:` 混用，禁止 `ˈ` 与 `'` 混用

- **严格单词验证规则**：限制单词字符集范围
  - 仅允许26个大小写字母：A-Z, a-z
  - 保留连字符、缩写点、所有格：`- . '`
  - 限制变音符号为指定集合：`é ü ñ ç à è ì ò ù â ê î ô û ä ë ï ö ü ÿ æ œ ø`
  - 移除对其他欧洲语言字符的宽松支持

- **音标长度限制优化**：
  - 将音标最大长度从50字符扩展到100字符
  - 支持长单词音标识别，如 `pneumonoultramicroscopicsilicovolcanoconiosis`

- **问题修复**：
  - 修复音标字符集缺失 `θ`、`ð`、`ŋ` 等关键IPA符号
  - 解决 `[invalid]` 被错误识别为有效音标的问题
  - 修复长音标因长度限制无法识别的问题

### 测试和验证
- 创建专门的音标验证测试页面 `strict_phonetic_test.html`
- 创建单词验证测试页面 `strict_word_test.html`
- 预处理模块全面测试通过，验证规则严格性和准确性
- 修复测试页面中JavaScript字符串转义问题

## [v1.2.5] - 2025-06-09
- 修复test.html中缺少fileStorage.js引用导致的FileStorageManager未定义错误
- 修复模块3清空按钮逻辑问题：
  - 之前：清空按钮会删除所有新建的textarea，只保留一个空的默认textarea
  - 现在：清空按钮只清空所有textarea的内容，保留用户创建的所有textarea
- 优化测试页面的用户体验，避免意外删除用户创建的输入框
- 提升测试环境的功能稳定性和可用性
